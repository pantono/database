...
class MysqlQueryDeleteTest extends AbstractMysqlAdapterTestCase
{
    public function testSimpleDelete()
    {
        $update = new Delete('table', ['test=?' => 2], $this->db);
    ...
    {
        $update = new Delete('table', ['test=?' => 2, 'test3=?' => 4], $this->db);
    ...
    {
        $update = new Delete('table', ['test in (?)' => [2, 4, 5]], $this->db);
    ...
    {
        $update = new Delete('table', ['test in (1,2,3)'], $this->db);
