...
class MssqlQuerySelectTest extends AbstractMssqlAdapterTestCase
{
    public function testSimpleSelectMssql(): void
    {
        $select = (new MssqlSelect($this->db))->from('table')->where('test_column = ?', 'test');
    ...
    {
        $select = (new MssqlSelect($this->db))->from('table')->joinRight('joined_table', 'joined_table.id=table.join_id', ['id'])->where('test_column = ?', 'test');
    ...
    {
        $select = (new MssqlSelect($this->db))->from('res_reservation')
            ->order('start_time')
    ...
    {
        $select = (new MssqlSelect($this->db))->from('table')->joinInner('joined_table', 'joined_table.id=table.join_id', ['id'])->where('test_column = ?', 'test')->limit(10);
    ...
    public function testNoOrderThrowsException()
    {
        ...
        $select = (new MssqlSelect($this->db))->from('table')->limitPage(1, 10);
        $select->__toString();
    }
